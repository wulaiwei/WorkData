...
//打开一个最大化的Dialog
function ShowMaxDialog(tit, msgcontent) {
    dialog({
        title: tit,
        content: msgcontent,
        okValue: '确定',
...
function ShowOkCancelDialog(category) {
    top.dialog({
        title: '模板库',
        url: '/Admin/Category/LoadTemplateLibrary',
        width: 600,
        id: 'TemplateLibrary',
        onclose: function () {
            var html = this.returnValue; //获取返回值
            $('#' + category).val(html);
        }
    }).showModal();
}
    ...
    var singleCheckbox = function (parentObj) {
        ...
        newObj.click(function () {
            if ($(this).hasClass("selected")) {
                ...
                $(this).parent().find(".CheckUpdateValue").val(false);
                //$(".CheckUpdateValue").val(0);
            } else {
                $(this).addClass("selected");
                //$(".CheckUpdateValue").val(1);
                $(this).parent().find(".CheckUpdateValue").val(true);
            }
    ...
    var singleSelect = function (parentObj) {
        ...
        selectObj.find("option").each(function (i) {
            ...
            liObj.click(function () {
                ...
                divObj.parent().find(".UpdateSelectValue").val(selectObj.find("option:selected").val());
            });
        });
        //设置样式
