namespace System.Data.Entity.Caching
{
    ...
    /// <remarks>
    /// <para>
    /// This CacheManager uses the <see cref="Locator" /> container to resolve an <see cref="ICacheProvider" /> to store the
    /// cache items.
    /// </para>
    /// <para>
    /// The CacheManager also supports tagging the cache entry to support expiring by tag. <see cref="CacheKey" /> supports a
    /// list
    /// of tags to associate with the cache entry.  Use <see cref="F:Expire" /> to evict the cache entry by
    /// <see cref="CacheTag" />.
    /// </para>
    /// </remarks>
    /// <example>
    ...
    public class CacheManager
    {
        ...
        /// <returns>
        /// <c>true</c> if insertion succeeded, or <c>false</c> if there is an already an entry in the cache that has the same key
        /// as key.
        /// </returns>
        ...
        /// A span of time within which a cache entry must be accessed before the cache entry is
        ...
        /// <summary>
        /// Gets the cache value for the specified key that is already in the dictionary or the new value if the key was not in the
        /// dictionary.
        /// </summary>
        /// <param name="key">A unique identifier for the cache entry.</param>
        /// <param name="value">The object to insert.</param>
        ...
        /// <summary>
        /// Gets the cache value for the specified key that is already in the dictionary or the new value for the key as returned
        /// by <paramref name="valueFactory" />.
        /// </summary>
        /// <param name="key">A unique identifier for the cache entry.</param>
        ...
        /// <summary>
        /// Gets the cache value for the specified key that is already in the dictionary or the new value for the key as returned
        /// asynchronously by <paramref name="valueFactory" />.
        /// </summary>
        /// <param name="cacheKey">A unique identifier for the cache entry.</param>
        ...
        /// or the new value for the key as returned by <paramref name="valueFactory" /> if the key was not in the dictionary.
        /// </returns>
        public virtual Task<object> GetOrAddAsync(CacheKey cacheKey, Func<CacheKey, Task<object>> valueFactory,
        ...
        protected ICacheProvider ResolveProvider()
        {
            ...
            if (provider == null)
                throw new InvalidOperationException(
                    "Could not resolve the ICacheProvider. Make sure ICacheProvider is registered in the Locator.Current container.");
