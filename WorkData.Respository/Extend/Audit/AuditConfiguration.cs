...
namespace System.Data.Entity.Audit
{
    ...
    public class AuditConfiguration
    {
        ...
        private readonly ConcurrentDictionary<Type, IMemberAccessor> _displayCache =
            new ConcurrentDictionary<Type, IMemberAccessor>();

        private readonly ConcurrentDictionary<string, IMethodAccessor> _formatterCache =
            new ConcurrentDictionary<string, IMethodAccessor>();
        ...
        /// <summary>
        /// Gets or sets a value indicating whether the log is maintain across multiple calls to
        /// <see cref="M:System.Data.Entity.DbContext.SaveChanges" />.
        /// Each additional call will add to the <see cref="P:System.Data.Entity.Audit.AuditLogger.LastLog" /> instance.
        /// </summary>
        /// <value>
        ...
        {
            var fullName = entityType.FullName + "." + name;
        ...
        internal IMemberAccessor GetDisplayMember(Type entityType)
        {
            ...
            {
                var typeAccessor = TypeAccessor.GetAccessor(entityType);
        ...
        private static readonly Lazy<AuditConfiguration> _default =
            new Lazy<AuditConfiguration>(() => new AuditConfiguration());
        ...
        public class AuditEntityConfiguration<TEntity>
        {
            ...
            /// <returns></returns>
            public AuditEntityConfiguration<TEntity> NotAudited<TProperty>(
            ...
            /// <param name="propertyExpression">The property expression.</param>
            /// <returns></returns>
            public AuditEntityConfiguration<TEntity> AlwaysAudited<TProperty>(
                Expression<Func<TEntity, TProperty>> propertyExpression)
            {
                var property = ReflectionHelper.ExtractPropertyInfo(propertyExpression);
                var key = property.DeclaringType.FullName + "." + property.Name;
            ...
            /// <returns></returns>
            public AuditEntityConfiguration<TEntity> FormatWith<TProperty>(
                Expression<Func<TEntity, TProperty>> propertyExpression,
                Expression<Func<AuditPropertyContext, object>> formatMethod)
            {
            ...
            /// <param name="propertyExpression">The property expression.</param>
            /// <returns></returns>
            public AuditEntityConfiguration<TEntity> DisplayMember<TProperty>(
                Expression<Func<TEntity, TProperty>> propertyExpression)
            {
                var type = typeof(TEntity);
            ...
            private static IMethodAccessor ExtractMethodInfo(
                Expression<Func<AuditPropertyContext, object>> methodExpression)
            {
            ...
            private static IMemberAccessor ExtractMember<TProperty>(
                Expression<Func<TEntity, TProperty>> propertyExpression)
            {
