...
{
    /// <summary>
    /// A class to store the collection of <see cref="FutureContext" /> class that are linked to an
    ...
    public class FutureStore
    {
        ...
        public IFutureContext GetOrCreate(ObjectQuery objectQuery)
        {
            ...
            var key = RuntimeHelpers.GetHashCode(objectContext);
        ...
        {
            var countCollected = _futureContext.Values.Count(x => !x.IsAlive);
            var diff = (double)countCollected / _futureContext.Count;
